export function mergeOptions(initialOptions, customOptions) {
  ...
  for (const property in initialOptions) {
    if (merged.hasOwnProperty(property)) {
      if (
        initialOptions[property] !== null &&
        initialOptions[property].constructor === Object
      ) {
        merged[property] = mergeOptions(
          initialOptions[property],
          merged[property]
        );
      }
    } else {
      merged[property] = initialOptions[property];
    }
  }
  return merged;
}
...
export const toast = (() => {
  ...
  const DEFAULT_SETTINGS = {
    style: {
      main: {
        background: "rgba(0, 117, 201, .65)",
        "box-shadow": "0 0 10px rgba(0, 117, 201, .8)",
